      ...
      <main className="flex-1 bg-white rounded-2xl shadow p-4 lg:p-6">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-6 font-semibold text-gray-600">
            <button className="text-black border-b-2 border-teal-500 pb-1">Recommended</button>

            <Link href="/user/homepage/booklist" className="text-sm text-teal-600 hover:underline font-medium">
              See All
            </Link>
          </div>
          ...
          <div className="flex items-center bg-gray-100 px-4 py-2 rounded-xl gap-2 w-full lg:w-[270px]">
            ...
            <input className="bg-transparent outline-none text-sm w-full" placeholder="Search for title, author, tags, etc" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {books.slice(0, 3).map(book => (
              ...
              >
                <img src={`/buku/coverbuku/${book.gambar}`} alt={book.judul} className="w-full h-100 object-cover" />

                <CardContent className="p-3">
                  <h3 className="font-semibold text-gray-800 text-sm truncate">{book.judul}</h3>
